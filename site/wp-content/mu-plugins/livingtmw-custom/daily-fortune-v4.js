(function () {
	...
	var myUi = {
		...
		'운세 기준': 'ကံကြမ္မာတွက်ချက်မည့် အချက်အလက်', '생년월일': 'မွေးသက္ကရာဇ်', '띠 선택': 'တိရစ္ဆာန်နှစ်ကို ရွေးမည်',
	...
	form.addEventListener('submit', function (event) {
		...
		var birthKey = 'direct';
		var label = language === 'my' ? 'တိရစ္ဆာန်နှစ်ကို ရွေးထားသည်' : '띠 선택';
		...
		if (method === 'birth') {
			...
			if (!zodiacYear) {
				result.innerHTML = '<p class="livingtmw-fortune__error">' + (language === 'my' ? 'ဤဘရောက်ဇာတွင် နေပြက္ခဒိန်မှ တိရစ္ဆာန်နှစ်ကို မတွက်နိုင်ပါ။ တိရစ္ဆာန်နှစ်ကို ကိုယ်တိုင်ရွေးပါ။' : '이 브라우저에서는 양력의 띠 계산을 지원하지 않습니다. ‘띠 선택’을 이용해 주세요.') + '</p>';
				return;
			}
